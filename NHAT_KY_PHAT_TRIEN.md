...
3. **Task 1.3 — Mở rộng bộ dữ liệu kiểm nghiệm (Test Dataset 100+ câu):** Tăng quy mô từ 30 lên 100 câu hỏi kèm gold SQL, phân loại chi tiết theo độ khó (easy/medium/hard/complex).
...
1. **Xây dựng module kiểm tra ngữ pháp & cấu trúc [`sql_validator.py`](file:///d:/UIT/DATN/official/sql_validator.py):**
   - Khai báo danh sách tuyệt đối `VALID_TABLES` cho 12 bảng trong MIMIC-IV subset (`patients`, `admissions`, `diagnoses_icd`, `labevents`, `prescriptions`, v.v.).
...
### 🔹 [Phase 1 - Task 1.3] Mở rộng bộ dữ liệu kiểm nghiệm Test Dataset lên 100 Câu
**Trạng thái:** ✅ Hoàn thành  
**Các file tác động:** `expand_test_dataset.py` (tạo mới), `test_dataset.json` (cập nhật từ 30 → 100 câu)
...
#### 2. Công việc thực hiện (What & How?)
1. Viết script tạo dữ liệu tự động [`expand_test_dataset.py`](file:///d:/UIT/DATN/official/expand_test_dataset.py) chứa 70 cặp `(question_vi, gold_sql, tables, difficulty)` mới (`t031` đến `t100`), được thiết kế tỉ mỉ theo đúng chuẩn MIMIC-IV 12 bảng.
2. **Phân bố độ khó chuẩn học thuật của 100 câu trong [`test_dataset.json`](file:///d:/UIT/DATN/official/test_dataset.json):**
...
3. **Phân bố bao phủ toàn diện 12 bảng MIMIC-IV:**
...
#### 1. Hiện trạng
- Các script `evaluate.py` đã sẵn sàng để chạy thử nghiệm trên 100 câu hỏi mới của `test_dataset.json`.
...
- Ngay khi Docker Desktop được bật lên và container `mimic-postgres` (bảng MIMIC-IV) hoạt động, chúng ta sẽ chạy lệnh:
  ...
  ```
- Kết quả thu được sẽ cho ta số liệu **Valid SQL Rate (VSR)** và **Execution Accuracy (EX)** chính thức trên bộ 100 câu làm mốc chuẩn (Baseline) cho toàn bộ đồ án!
